...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between gap-4 py-5 text-left text-base font-bold transition-colors hover:text-primary [&[data-state=open]>span>svg]:rotate-45",
        className
      )}
      {...props}
    >
      ...
      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-border">
        <Plus className="h-4 w-4 text-secondary transition-transform duration-300" />
      </span>
    </AccordionPrimitive.Trigger>
